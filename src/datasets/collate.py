...
def collate_fn(dataset_items: list[dict]):
    ...
    specs_lens = torch.tensor(
        [item["spectrogram"].shape[-1] for item in dataset_items]
    )  # list [1, 1]

    phases = [item["phase"].squeeze(0).transpose(0, 1) for item in dataset_items]
    ...
    specs_batch = torch.nn.utils.rnn.pad_sequence(
        specs, batch_first=True, padding_value=0.0
    ).transpose(-2, -1)
    phases_batch = torch.nn.utils.rnn.pad_sequence(
        phases, batch_first=True, padding_value=0.0
    ).transpose(-2, -1)
    ...
    video_batch = torch.stack(
        [
            torch.stack(
                [
                    (
                        torch.tensor(item["mouth1"])
                        if item["mouth1"] is not None
                        else torch.zeros_like(torch.tensor(item["mouth2"]))
                    ),
                    (
                        torch.tensor(item["mouth2"])
                        if item["mouth2"] is not None
                        else torch.zeros_like(torch.tensor(item["mouth1"]))
                    ),
                ]
            )
    ...
    for source_idx in range(n_sources):
        source_specs = [
            item["sources"][source_idx]["spectrogram"].squeeze(0).transpose(0, 1)
            for item in dataset_items
        ]
        source_audio = [
            item["sources"][source_idx]["audio"].squeeze(0) for item in dataset_items
        ]
        source_phases = [
            item["sources"][source_idx]["phase"].squeeze(0).transpose(0, 1)
            for item in dataset_items
        ]
        ...
        source_specs_batch = torch.nn.utils.rnn.pad_sequence(
            source_specs, batch_first=True, padding_value=0.0
        ).transpose(-2, -1)
        source_phases_batch = torch.nn.utils.rnn.pad_sequence(
            source_phases, batch_first=True, padding_value=0.0
        ).transpose(-2, -1)
