...
from src.model.DTTNetPlus.blocks.block_fusion import FusionBlock
from torch.utils.checkpoint import checkpoint


class LatentModule(nn.Module):
    def __init__(self, fc_dim, n_heads=1, n_channels=256, n_layers=3, video_embed_dim=128):
        super().__init__()
        ...
        self.fusion_block = FusionBlock(
            ...
            audio_freq=fc_dim,
            video_channels=video_embed_dim,
        )
    ...
    def forward(self, x, video_encoded):
        x = self.tfc_tdf(x)
        x = self.fusion_block(x, video_encoded)
